...
class TheLedger():
    ...
    def __init__(self):
        ...
        # Consensus dan sonra valid içine atılır. Tek olcak
        self.valid_submit = []
    ...
    def create_new_block(self, proof, previous_hash=None):
        ...
        '''
        block = {
            'index': len(self.current_submits) + 1,
            'proof': proof,
            'previous_hash': previous_hash or self.hashing_block(self.current_submits[-1]),
...
def mine():
    ...
    # Guide sayısı kadar shared keys oluşturulur
...
def validation(server,miner):
    ...
    if (  server_lp[0] == miner_lp[0] 
            & server_lp[1] == miner_lp[1] ):
        True
...
def new_submit():
    ...
    index = theLedger.new_submit(values['vuln_name'], values['type'], values['description'], 
    values['CVSS'], values['platform'])

    response = {'message': f'The new submit will be added to Tree {index}'}
...
def full_tree():
    ...
    # Dict formatında bu şekilde çalışır mı?
    response = {
